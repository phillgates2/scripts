...
-----------------------------------------------------------------------------
 ...
 (the USE button, default E key) while standing right next to them:
   ...
   stand within KICK_RANGE of the projectile, look at it and use it ->
 ...
 HOW IT WORKS
   ...
   * The Lua API does not expose the raw usercmd button state, so the
     USE press is approximated by the player being in contact range of
     the projectile and looking at it (the same geometry the engine's
     own "use" action traces). A per-projectile cooldown prevents
     continuous re-kicking while you keep looking at it.
 ...
 INSTALL
   ...
   3. Restart the map/server. On load the console prints:
        [kick_projectiles] loaded: stand next to a grenade/canister, look at it and use it to kick it
=============================================================================
]]--
...
local KICK_POWER        = 380    -- horizontal kick speed (units/sec)
local KICK_UP           = 220    -- extra upward speed so it hops off the ground
...
-- weapons that can be kicked (weapon ids, bg_public.h)
...
-- collects the live players (on a team and not dead) into players[]
...
local function kick_projectile(p, pos, pl, levelTime)
	...
	if KICK_SOUND and kick_sound_index > 0 then
...
function et_RunFrame(levelTime)
	...
	-- 3. for each projectile, look for a player close by and looking at it
...
function et_InitGame(levelTime, randomSeed, restart)
	...
	if KICK_SOUND then
		kick_sound_index = et.G_SoundIndex(KICK_SOUND_FILE)
		if not kick_sound_index or kick_sound_index == 0 then
			kick_sound_index = 0
		end
	end
	log("loaded: stand next to a grenade/canister, look at it and use it to kick it")
end
